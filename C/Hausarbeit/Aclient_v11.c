		...
		if (FD_ISSET(0, &bereit)) {
			...
			*(buffer+rc) = (char) 0;
			...
			/* Prüfen auf blanks im Namen 
			   if( sscanf(buffer,"%s", playername) != 1) {
			   printf("Bitte einen gültigen Spielernamen angeben: ");
			   continue;
			   }
			 */

			/*Nach erfolgreichem Einlesen den Namen nochmal ausgeben */
			/*printf("Angegebener Name: %s", playername);*/
			strcpy(playername, buffer);
			/* Namen senden */
			str_senden(sd, "NAME: ", playername, rc);
			/* Aus der Einleseschleife ausbrechen */
			break;
		}
	}
		...
		while(1) {
			...
			if (FD_ISSET(0, &bereit)) {
				...
				*(secret+rc) = (char) 0; /* String erstellen */
				...
				printf("%s\n", secret);
				/* Geheimzahl senden */
				str_senden(sd, "GEHEIMZAHL: ", secret, 5);
				break;
			}
		}
