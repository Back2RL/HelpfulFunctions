...
sum += log((double) i);
    ...
    const int max{2};
    byte** numbers{nullptr};
    try {
        numbers = (byte **) malloc(max * sizeof(byte *));
        for(int cnt{0};cnt < max;++cnt) {
            printf("%d\n",cnt);
            numbers[cnt] = new byte[1024 * 1024 * 1024];
        }
    } catch (bad_alloc e) {
    ...
    for(int cnt{0};cnt < max;++cnt) {
        delete_safe_array(numbers[cnt]);
    }
    delete_safe_array(numbers);
