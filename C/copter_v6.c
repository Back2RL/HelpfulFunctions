...
bsize = scanDouble("Akkukapazität [mAh]:");
		for ( n = 1; n <= nmax; n++ ) {
			if ( (n % 2) != 0) {
			...
			printf("Iges( %2d ) = %4.1lfA\t\tm( %2d ) = %5.0lfg\t\t Schwebeflugdauer [20 Prozent Restkap.] = %5.1lfmin\n", n, I*n, n, mges*1000, ((0.8 * (bsize / 1000)) / (I*n) ) * 60 );			
			/*	power = mges / (Fgmax * n);
		...
		scanString("Nochmal? (j/n) : ", again, MAXLEN);
