	...
	private static void checkAnimationSettings() {
		try {
			...
			if (settingsFile.exists()) {
				...
				System.out.println("***");
				System.out
						.println("*** DrawingPanel saving animated GIF: " + new File(animationSaveFileName).getName());
				System.out.println("***");
				settingsFile.delete();
	...
	*/
	...
	public DrawingPanel(int width, int height) {
		...
		synchronized (getClass()) {
			instances++;
			instanceNumber = instances; // each DrawingPanel stores its own int
										// number
		...
		if (DEBUG)
			System.out.println("w=" + width + ",h=" + height + ",anim=" + isAnimated() + ",graph=" + isGraphical()
					+ ",save=" + shouldSave());
	...
	private void compareToURL() {
		...
		try {
			...
			if (filenames.isEmpty()) {
				JOptionPane.showMessageDialog(frame, "No valid web files found to compare against.",
				...
				if (filenames.size() == 1) {
					...
					int choice = showOptionDialog(frame, "File to compare against?", "Choose File",
	...
	private BufferedImage getImage() {
		...
		if (DEBUG)
			System.out.println("getImage setting background to " + backgroundColor);
		g.setColor(backgroundColor);
	...
	private String saveAsHelper(String extension) {
		...
		if (new File(filename).exists() && JOptionPane.showConfirmDialog(frame, "File exists.  Overwrite?",
	...
	private class DiffImage extends JPanel implements ActionListener, ChangeListener {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if (source == box) {
				...
				Color color = JColorChooser.showDialog(frame, "Choose highlight color", highlightColor);
				if (color != null) {
		...
		// public void diff(InputStream input1, InputStream input2) throws
		// IOException {
		// diff(ImageIO.read(input1), ImageIO.read(input2));
		// }
		...
		public void save(File file) throws IOException {
			...
			BufferedImage img = new BufferedImage(getPreferredSize().width, getPreferredSize().height,
					BufferedImage.TYPE_INT_ARGB);
		...
		public void setImage1(BufferedImage image) {
			...
			image1 = image;
			setPreferredSize(new Dimension(Math.max(getPreferredSize().width, image.getWidth()),
